...
@dataclass
class ChangePointDetection(DetectionMethods):
    ...
    def get_estimators(self) -> str:
        ...
        s = "Estimators: "
        for key in ChangePointDetection.estimators:
            s += f"{key}: use with one of {
                ChangePointDetection.possible_arguments[key]}\n"
    ...
    def get_models(self) -> str:
        ...
        s = f"Models: {ChangePointDetection.models}"
        return s
    ...
    def get_kernels(self) -> str:
        ...
        s = f"Kernels: {ChangePointDetection.kernels}"
        return s
    ...
    def detect_change_point(self, samples: ArrayLike | None = None,
        ...
        if samples is not None:
            self.samples = np.array(samples)
        assert type(self.samples) is NDArray
        ...
        self.optimal_breakpoints = ChangePointDetection.estimators[estimator](
            **est_args).fit(self.samples).predict(**fit_args)

        if len(self.optimal_breakpoints) > 1:
            self.detected = True

    def display(self, save_fig: bool = True, figname: str = "test",
                filetype: str = "pdf", use_ax: plt.Axes | None = None) -> None:
        """
